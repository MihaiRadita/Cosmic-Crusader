...
namespace ratchet
{
	...
	void AnimationJumpRun::invertAnimationFramesList(Weapon::TYPE& weaponused, WeaponAnimation::ANGLE& angle, WeaponAnimation::STATE& state, bool invert)
	{
		if (!m_repeatAnimation)
			return;
		...
		if (weaponused != Weapon::TYPE::None)
		{
			...
			auto& frames = m_weaponAnimationFramesMap[weaponused][angle][state];
