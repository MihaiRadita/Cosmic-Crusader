...
namespace ratchet
{
	void AnimationDeath::playAnimation(sf::Sprite& sprite, Weapon::TYPE& weaponUsed, WeaponAnimation::ANGLE& angle, WeaponAnimation::STATE& state)
	{
		if (!m_initialTexture)
		{
			m_currentFrameIndex++;
		...
		if (m_currentFrameIndex == 0)
		{
			if (m_isAnimTransition)
			{
				...
				TRACE_CHANNEL("ANIMATION", "PLayer Death image ", m_currentFrameIndex);
#endif
	...
	AnimationDeath::AnimationDeath(std::string& texturePath, std::vector<Weapon::TYPE>& usableWeaponTypeList)
	{
		initVariables();
	...
	void AnimationDeath::initVariables()
	{
	...
	void AnimationDeath::resetCurrentAnimIndex()
	{
		m_currentFrameIndex = 0;
	}

	void AnimationDeath::resetPlayerAnimTimer()
	{
		m_animationTimer.restart();
		m_animationSwitch = true;
	}
	void AnimationDeath::setAnimationSwitch(bool animSwitch)
	{
		m_animationSwitch = animSwitch;
	}

	void AnimationDeath::invertAnimationFramesList(Weapon::TYPE& weaponused, WeaponAnimation::ANGLE& angle, WeaponAnimation::STATE& state, bool invert)
	{
		if (!m_repeatAnimation)
			return;
	...
	int AnimationDeath::getAnimSize(Weapon::TYPE& type, WeaponAnimation::ANGLE& angle, WeaponAnimation::STATE& state)
	{
	...
	int AnimationDeath::getCurrentAnimIndex()
	{
		return m_currentFrameIndex;
	}

	bool AnimationDeath::getAnimationSwitch()
	{
